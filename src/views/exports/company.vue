<template>
	<div class="doctor">
		...
		<div class="doctor-table">
			<el-table :data="tableData" border style="width: 1531px">
			    <el-table-column
			        prop="code"
			        label="药企编码"
			        width="130">
...
<script>
	import {selectDrugCompany,selectDrugCompanyExcel } from '@/api/export'
	export default{
		name: '',
		data(){
			return{	
		...
		methods: {
			selectDrugCompany(page,limit){
				...
				selectDrugCompany(params).then(response => {
			...
			companySearch(){
				...
				this.selectDrugCompany(this.listQueryPage,this.handleSizeChangeData);
				
			},
			handleSizeChange(val){
				this.handleSizeChangeData = val;
				this.selectDrugCompany(this.listQueryPage,val);
			},
			handleCurrentChange(val){
				this.listQueryPage = val;
				this.selectDrugCompany(val,this.handleSizeChangeData);
			},
			timeData(value){
				if(value == '') return '';
			...
			companyExcel(){
				window.location.href = process.env.BASE_API + '/exports/exportsDrugCompany?type='+this.typeSelect+'&beginTime='+this.startTime+'&endTime'+this.endTime;
			}
		},
		mounted: function(){
			this.selectDrugCompany(this.listQueryPage,this.handleSizeChangeData);
